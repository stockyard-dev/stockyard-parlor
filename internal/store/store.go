...
func(d *DB)Update(e *Conversation)error{_,err:=d.db.Exec(`UPDATE conversations SET visitor_name=?,visitor_email=?,subject=?,messages=?,status=?,assignee=?,page_url=? WHERE id=?`,e.VisitorName,e.VisitorEmail,e.Subject,e.Messages,e.Status,e.Assignee,e.PageURL,e.ID);return err}
...
func(d *DB)Search(q string, filters map[string]string)[]Conversation{
    where:="1=1"
    args:=[]any{}
    if q!=""{
        where+=" AND (visitor_name LIKE ? OR subject LIKE ?)"
        args=append(args,"%"+q+"%");args=append(args,"%"+q+"%");
    }
    if v,ok:=filters["status"];ok&&v!=""{where+=" AND status=?";args=append(args,v)}
    rows,_:=d.db.Query(`SELECT id,visitor_name,visitor_email,subject,messages,status,assignee,page_url,created_at FROM conversations WHERE `+where+` ORDER BY created_at DESC`,args...)
    if rows==nil{return nil};defer rows.Close()
    var o []Conversation;for rows.Next(){var e Conversation;rows.Scan(&e.ID,&e.VisitorName,&e.VisitorEmail,&e.Subject,&e.Messages,&e.Status,&e.Assignee,&e.PageURL,&e.CreatedAt);o=append(o,e)};return o
}

func(d *DB)Stats()map[string]any{
    m:=map[string]any{"total":d.Count()}
    rows,_:=d.db.Query(`SELECT status,COUNT(*) FROM conversations GROUP BY status`)
