...
async function load(){var r=await fetch(A+'/conversations').then(function(r){return r.json()});items=r.conversations||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.visitor_name||'').toLowerCase().includes(q)||(i.visitor_email||'').toLowerCase().includes(q)||(i.subject||'').toLowerCase().includes(q)||(i.messages||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No items found.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1"><div class="item-title">'+esc(i.visitor_name)+'</div>';
if(i.visitor_email)h+='<div class="item-sub">'+esc(i.visitor_email)+'</div>';
...
if(i.subject)h+='<span>'+esc(i.subject)+'</span>';
if(i.messages)h+='<span>'+esc(i.messages)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.assignee)h+='<span>'+esc(i.assignee)+'</span>';
if(i.page_url)h+='<span>'+esc(i.page_url)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/conversations/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{visitor_name:"",visitor_email:"",subject:"",messages:"",status:"",assignee:"",page_url:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' PARLOR</h2>';
h+='<div class="fr"><label>Visitor Name *</label><input id="f-visitor-name" value="'+esc(i.visitor_name)+'"></div>';
h+='<div class="fr"><label>Visitor Email</label><input id="f-visitor-email" value="'+esc(i.visitor_email)+'"></div>';
h+='<div class="fr"><label>Subject</label><input id="f-subject" value="'+esc(i.subject)+'"></div>';
h+='<div class="fr"><label>Messages</label><input id="f-messages" value="'+esc(i.messages)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>Assignee</label><input id="f-assignee" value="'+esc(i.assignee)+'"></div>';
h+='<div class="fr"><label>Page U R L</label><input id="f-page-url" value="'+esc(i.page_url)+'"></div>';
...
async function submit(){var body={visitor_name:document.getElementById('f-visitor-name').value.trim(),visitor_email:document.getElementById('f-visitor-email').value.trim(),subject:document.getElementById('f-subject').value.trim(),messages:document.getElementById('f-messages').value.trim(),status:document.getElementById('f-status').value.trim(),assignee:document.getElementById('f-assignee').value.trim(),page_url:document.getElementById('f-page-url').value.trim()};
if(editId){await fetch(A+'/conversations/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/conversations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
